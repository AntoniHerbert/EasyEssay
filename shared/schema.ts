...
// Rubric category schema for custom scoring (legacy alias)
// Now uses RubricItem schema for consistency
export const rubricCategorySchema = z.object({
  id: z.string().optional(), // Optional for backwards compatibility
  name: z.string(),
  maxScore: z.number(),
...
export const inspirations = pgTable("inspirations", {
  ...
  content: text("content").notNull(),
  category: varchar("category", { length: 50 }).notNull(), // literature, philosophy, science, history, etc.
  type: varchar("type", { length: 20 }).notNull(), // book, article, quote, excerpt
  source: text("source"), // publication, website, etc.
  tags: text("tags").array().default([]),
  difficulty: varchar("difficulty", { length: 20 }).notNull().default("intermediate"), // beginner, intermediate, advanced
  wordCount: integer("word_count").notNull().default(0),
  readTime: integer("read_time").notNull().default(5), // estimated reading time in minutes
...
export const userProfiles = pgTable("user_profiles", {
  ...
  streak: integer("streak").notNull().default(0), // consecutive days writing
...
export const userMessages = pgTable("user_messages", {
  ...
  type: varchar("type", { length: 20 }).notNull().default("text"), // text, correction_feedback, friend_request
  relatedEssayId: varchar("related_essay_id"), // for correction feedback
...
// Correction object structure (stored in JSONB)
// Category is now dynamic - must match a name from the essay's rubric
export const correctionSchema = z.object({
  category: z.string(), // Dynamic: matches rubric category names
...
// RubricItem - defines a single evaluation criterion template
// Stored in essays.rubric column
...
// RubricScore - stores the result of an evaluation for a specific criterion
// Stored in peer_reviews.rubricScores column
export const rubricScoreSchema = z.object({
  categoryName: z.string(), // Must match a RubricItem.name from essay's rubric
...
export const peerReviews = pgTable('peer_reviews', {
  ...
  grammarScore: integer('grammar_score').notNull().default(100), // 0-200
  styleScore: integer('style_score').notNull().default(100), // 0-200
  clarityScore: integer('clarity_score').notNull().default(100), // 0-200
  structureScore: integer('structure_score').notNull().default(100), // 0-200
  contentScore: integer('content_score').notNull().default(100), // 0-200
  researchScore: integer('research_score').notNull().default(100), // 0-200
  overallScore: integer('overall_score').notNull().default(600), // Sum of all categories (0-1200)
...
// Insert schemas for peer review
...
export const communities = pgTable('communities', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  code: varchar('code', { length: 8 }).notNull().unique(), // Shareable code like "ABC123"
...
// Insert schemas for communities
...
// Payload schemas for each content type
